...
tree_dat_temp <- root_2_tips(tree_dat_temp, trait_var = 'trait_A')

```

To produce a fully resolved parsimonious solution, `tips_2_root()` can be followed by `root_2_tips()`. It is possible that `root_2_tips()` will generate the same solution following `tips_2_root()` with `prefer_intersect == FALSE` or `prefer_intersect == TRUE`, though if they do differ, `root_2_tips()` following `tips_2_root()` with `prefer_intersect == TRUE` will generally result in a more parsimonious solution.

```{r}
# First, with prefer_intersect == FALSE
# By default, tips_2_root() returns a list of option at internal nodes only
# To return a parsimony solution, use 
tree_dat_temp <- tips_2_root(tree_dat, trait_var = 'trait_A',
                             prefer_intersect = FALSE)
...
tree_dat_temp <- does_trait_change(tree_dat_temp,  trait_var = 'trait_A')
cat('Parsimony score: ',
    sum(tree_dat_temp$trait_change == TRUE, na.rm = T))

# Second, with prefer_intersect == TRUE
tree_dat_temp <- tips_2_root(tree_dat, trait_var = 'trait_A',
                             prefer_intersect = TRUE)
tree_dat_temp <- root_2_tips(tree_dat_temp,trait_var = 'trait_A')
...
The function `parsimony_score()` function packages up these last few steps, running `tips_2_root()` (with `prefer_intersect = TRUE`, by default), then `root_2_tips()`, before counting the number of trait changes picked up by `tree_dat_temp()`. Compare trait A explored above with trait B which was designed to show greater phylogeny-trait correlation.
...
ggtree(tree, aes(col = tree_dat_temp$trait_B)) +
  geom_point(data = tree_dat_temp,
             aes(col = trait_B, shape = trait_B), size = 2) +
  scale_color_manual(values = c('firebrick2', 'dodgerblue'), name = 'Trait B') +
